# Original code from https://github.com/pgrafov/python-avl-tree
...
class AVLTree(object):
    ...
    def find(self, key, limit=-1):
        return self.find_in_subtree (self.rootNode, key, limit)
    
    def find_in_subtree (self, node, key, limit=-1):
        if node is None:
            return []  # key not found
        
        if key < node.key:
            return self.find_in_subtree(node.leftChild, key, limit)
        elif key > node.key:
            return self.find_in_subtree(node.rightChild, key, limit)
            ...
            return [node] + self.find_in_subtree(node.rightChild, key, lc)
